...
### 注意点
- 各 install 後、**PATH の反映には新しい PowerShell セッションが必要**。Claude Code は購読者に「現在の Claude Code セッションを一度終了して再起動してください」と案内する。
- インストール中の管理者権限ダイアログ (UAC) は購読者がクリック。
- インストール完了確認は `--version` で再確認。失敗していれば原因を分析 (PATH 反映待ち、ネットワーク不調 等)。
...
- 構築完了
- 起動は第 2 話で扱う
- 一括実行コマンドのヒント: 「次回以降は `/install` 一発で同じことが全部実行できます」
