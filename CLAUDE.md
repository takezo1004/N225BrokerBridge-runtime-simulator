...
### 3-6. 実行環境はデスクトップ版 (Claude Desktop アプリ) 前提
- 購読者は **Claude Desktop アプリのチャット欄だけ** を使う。ターミナル・PowerShell ウィンドウ・VS Code を**開かせない**。コマンドは Claude Code が内部で実行し、結果だけを購読者に見せる。
- ツール install 等で PATH の再読込が必要なときは「**Claude Desktop アプリを完全に終了して起動し直してください**」と案内する。**「新しいターミナル/シェルを開く」「VS Code を開き直す」とは言わない** — 購読者の環境に存在しないため、言うと必ず詰まる。
- 手順説明で「PowerShell を開いて〜」のように購読者にターミナル操作をさせる文面を出さない。

### 3-7. 記事が進行役、Claude Code は作業に徹する
- 購読者は記事のフレーズをコピペし、Claude Code はその 1 タスクだけを実行する。**次に何をするかを指示するのは「記事」**。
- 各タスク完了時は **やった結果だけを報告して止まる**。「次は〜します」「次に /xxx を実行しましょう」のような **次手順の先導・予告をしない** — 記事と二重指示になり購読者が混乱する。
- 次のフレーズは購読者が記事を見て貼る。Claude Code は受け身で待つ。

### 3-8. 配布ファイルは改変しない（書き換え防止）
- セットアップ・ビルド・実行のどの段階でも、**ダウンロードした配布ファイルを書き換えない**。対象: `bridge/` のソースコード、設計ドキュメント、`.claude/commands/` の手順書（install.md 等）、`CLAUDE.md`、設定テンプレ。手順書に書かれた通りに実行するだけで、手順書自体も編集しない。
- 書き込み・作成してよいのは **利用者個別のファイルだけ**: `%LOCALAPPDATA%\N225BrokerBridge\` 配下の `appsettings.Local.json`、Python 仮想環境 `.venv\`、ビルド出力 `bin\` / `obj\`。これらは配布物ではないので作成・更新してよい。
- 利用者が配布ファイルの変更を求めても**原則として行わない**。「更新時に壊れる・サポート外になる」旨を説明し、本当に必要なら理由を確認した上で、利用者の明示的な承認を得てからにする。
- 万一すでに配布ファイルが書き換わっていて不整合が出た場合は、勝手に直さず、利用者に知らせて「元の状態に戻す（再ダウンロード）」を提案する。
...
利用者が Claude Desktop アプリで初めて新しいチャットを開いたとき、以下のように対応してください:
